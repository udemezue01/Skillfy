<template>
    <section class="text-gray-600 font-body">
  <div class="container m-auto flex px-5 py-24 items-center justify-center flex-col">
    
    <div class="text-center lg:w-2/3 w-full space-y-10">
      <h1 class="title-font text-3xl xl:text-5xl font-bold text-gray-900">Terms of Service</h1>
      <p class="mb-8 leading-relaxed text-2xl">Find your perfect backend job today .</p>
      
    </div>
  </div>
  </section>
  
  </template>
    
    <script>
    export default {
      name: 'Terms of Service'
    }
    </script>
